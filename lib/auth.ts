      ...
      async authorize(credentials) {
        ...
        try {
          ...
          if (data.token) {
            ...
            // Get enrolled courses from JWT response (included by WordPress plugin)
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (token) {
        ...
        session.enrolledCourseIds = (token.enrolledCourseIds as number[]) || []

        console.log('[SESSION CALLBACK] Token enrolledCourseIds:', token.enrolledCourseIds)
        console.log('[SESSION CALLBACK] Session enrolledCourseIds set to:', session.enrolledCourseIds)
      }
      return session
    },
  },
