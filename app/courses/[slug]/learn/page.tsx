...
// Generate static params for all course learn pages at build time
export async function generateStaticParams() {
  console.log('[Build] Starting generateStaticParams for course learn pages...')
  ...
  try {
    ...
    console.log(`[Build] Fetched ${courses.length} courses for learn page static generation`)

    const params = courses.map((course) => ({
      slug: course.slug,
    }))

    console.log('[Build] Static params generation complete for course learn pages!')
    return params
  } catch (error) {
    console.error('[Build] Error generating static params for course learn pages:', error)
    return []
  }
}
